...
var tips: Array = [
	...
	"The BSOD (Blue Screen of Death) is a critical system error that indicates a serious problem with either your hardware or your drivers. To fix it, you'll need to analyze the stop code, check for recent changes, and systematically test your components to find the specific root cause of the instability.",
